...
def ft_draw_castle_map(tile_size):
    ...
    for i in range(len(castle_map)):
        for j in range(len(castle_map[i])):
            ...
            if dev:
                t.color("black")
                t.write(castle_map[i][j], font=("Arial", 10, "normal"))
                # turtle.color("blue")
...
def ft_update_player(coord):
    ...
    t.goto(BOARD_COORD[0] + player_coord[1] * size + text_center,
           BOARD_COORD[1] - player_coord[0] * size + BOARD_SIZE[1] - size)
    ...
    draw_tile((size, size), COULEUR_VUE)
    # Player drawing
    t.goto(BOARD_COORD[0] + player_coord[1] * size + size / 2 + text_center,
           BOARD_COORD[1] - player_coord[0] * size + BOARD_SIZE[1] - size - player_size + size / 2)
    player(player_size, COULEUR_PERSONNAGE)
    # Update
    s.update()
...
s.setup(WINDWOS_SIZE[0], WINDWOS_SIZE[1])  # 480x555 pixels
...
# Tile size
size = BOARD_SIZE[0] / len(castle_map[0]) if not BOARD_SIZE[0] / len(castle_map[0]) * len(castle_map) > BOARD_SIZE[1] \
    else BOARD_SIZE[1] / len(castle_map)
text_center = 0 if not BOARD_SIZE[0] / len(castle_map[0]) * len(castle_map) > BOARD_SIZE[1] \
    else (BOARD_SIZE[0] - size * len(castle_map[0])) / 2
